...
# ------------------------ TAB 2: ADMET ------------------------
with tab2:
    st.markdown("# 💊 ADMET Profile Viewer")
    st.markdown("Select a compound to view its pharmacokinetics and toxicity profile.")

    selected_cmp = st.selectbox("Choose a Compound", admet_df['Compound'].unique())

    admet_row = admet_df[admet_df['Compound'] == selected_cmp]

    if not admet_row.empty:
        st.markdown("### 🧪 ADMET Parameters")

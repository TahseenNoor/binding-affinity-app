...
if st.button("🔬 Predict Binding Affinity"):
    try:
        ...
        # Show the selected compound
        st.markdown(f"### 🧪 Selected Compound: `{selected_pair}`")
        ...
        for feat, score in sorted(feature_impact.items(), key=lambda x: -x[1]):
            st.markdown(
                f"<p>- <b>{feat}</b> plays a significant role in predicting binding affinity. Consider adjusting this parameter in experimental designs.</p>",
                unsafe_allow_html=True
            )
